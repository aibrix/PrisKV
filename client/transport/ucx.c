                                    ...
                                    void *arg)
{
    ...
    uint64_t resp_token = be64toh(resp->token);
    priskv_transport_req *ucx_req;

    priskv_log_debug(
        "Response request_id 0x%lx, status(%d) %s, length %d addr_offset %0x, token %0x\n",
        request_id, resp_status, priskv_resp_status_str(resp_status), resp_length, resp_addr_offset,
        resp_token);
    ...
    ucx_req->memory_region.token = resp_token;
                   ...
                   priskv_generic_cb usercb)
{
    ...
    if (ucs_unlikely(ucx_req->key == NULL)) {
        free(ucx_req);
        return NULL;
    }
    memcpy(ucx_req->key, key, keylen);
