...
class PriskvBenchmark:
    ...
    def op_zset(self):
        # 1. alloc
        status, region = self.client.alloc(self.shared_key, self.value_len)
        if status != 0:
            return status
        # 2. direct copy (memmove)
        ctypes.memmove(region.addr, self.shared_val_sgl.iova, self.value_len)
        # 3. seal
        return self.client.seal(self.shared_key, region)

    def op_zget(self):
        # 1. acquire
        status, region = self.client.acquire(self.shared_key)
        if status != 0:
            return status
        # 2. direct copy (memmove)
        ctypes.memmove(self.shared_val_sgl.iova, region.addr, region.length)
        # 3. release
        return self.client.release(self.shared_key, region)
