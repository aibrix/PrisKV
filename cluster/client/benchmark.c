...
static int random_gen_value(random_context *ctx, uint32_t *value_off, uint32_t *value_len)
{
...
static void random_add_kv(random_context *ctx, char *key, uint32_t value_off, uint32_t value_len)
{
    ctx->count++;
...
static void random_init(job_context *job, void **ctx)
{
    ...
    uint32_t value_off = 0, value_len = 0;
    char *key = NULL;
...
typedef struct {
    ...
    priskvClusterMemory *priskvmem;
} priskv_cluster_context;

static int priskv_cluster_drv_init(job_context *job, void **ctx)
{
    priskv_cluster_context **priskv_ctx = (priskv_cluster_context **)ctx;
    void *mem_pool;
    uint64_t len = 0;

    *priskv_ctx = malloc(sizeof(priskv_cluster_context));

    (*priskv_ctx)->job = job;

    (*priskv_ctx)->client = priskvClusterConnect(raddr, rport, "kvcache-redis");
    if (!(*priskv_ctx)->client) {
    ...
    if (!g_transfer && !g_temp_reg) {
        (*priskv_ctx)->priskvmem = priskvClusterRegMemory((*priskv_ctx)->client, (uint64_t)mem_pool, len,
                                                    (uint64_t)mem_pool, -1);
        if ((*priskv_ctx)->priskvmem == NULL) {
            return -1;
        }
...
static void priskv_cluster_drv_deinit(job_context *job, void *ctx)
{
    priskv_cluster_context *priskv_ctx = ctx;

    priskvClusterClose(priskv_ctx->client);
    if (!g_transfer && !g_temp_reg) {
        priskvClusterDeregMemory(priskv_ctx->priskvmem);
    }
    free(priskv_ctx);
}

static int priskv_cluster_drv_get_fd(void *ctx)
{
    priskv_cluster_context *priskv_ctx = ctx;
    return priskvClusterClientGetFd(priskv_ctx->client);
}

static void priskv_cluster_drv_handler(int fd, void *opaque, uint32_t events)
{
    priskv_cluster_context *priskv_ctx = opaque;
    priskvClusterClientProcess(priskv_ctx->client, 0);
}
...
typedef struct {
    ...
    void *cbarg;
    int ignore_no_such_key;
} priskv_cluster_req_context;

static void priskv_cluster_req_cb(priskvClusterStatus status, uint32_t valuelen, void *cbarg)
{
    priskv_cluster_req_context *ctx = (priskv_cluster_req_context *)cbarg;
    int ret = 0;

    if (status != PRISKV_CLUSTER_STATUS_OK) {
        if (!ctx->ignore_no_such_key || status != PRISKV_CLUSTER_STATUS_NO_SUCH_KEY) {
            ret = status;
        }
    }
...
static void priskv_cluster_drv_get(void *ctx, const char *key, void *value, uint32_t value_len,
                                 void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_context *priskv_ctx = ctx;
    priskvClusterSGL sgl;
    priskvClusterSGL *sgls;
    priskv_cluster_req_context *priskv_req_ctx;
    uint16_t nsgl;

    if (g_priskv_value_block_size && g_priskv_value_block_size < value_len) {
        nsgl = DIV_ROUND_UP(value_len, g_priskv_value_block_size);
        sgls = calloc(nsgl, sizeof(priskvClusterSGL));
        uint64_t offset = 0;
        for (int i = 0; i < nsgl; i++) {
            sgls[i].iova = (uint64_t)value + offset;
            sgls[i].length = priskv_min_u32(g_priskv_value_block_size, value_len - offset);
    ...
    priskv_req_ctx = malloc(sizeof(priskv_cluster_req_context));
    priskv_req_ctx->cb = cb;
    priskv_req_ctx->cbarg = cbarg;
    priskv_req_ctx->ignore_no_such_key = 0;

    priskvClusterAsyncGet(priskv_ctx->client, key, sgls, nsgl, priskv_cluster_req_cb, priskv_req_ctx);
...
static void priskv_cluster_drv_set(void *ctx, const char *key, void *value, uint32_t value_len,
                                 void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_context *priskv_ctx = ctx;
    priskvClusterSGL sgl;
    ...
    priskvClusterAsyncSet(priskv_ctx->client, key, sgls, nsgl, PRISKV_CLUSTER_KEY_MAX_TIMEOUT,
                        priskv_cluster_req_cb, priskv_req_ctx);
...
static void priskv_cluster_drv_del(void *ctx, const char *key, void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_context *priskv_ctx = ctx;
    priskv_cluster_req_context *priskv_req_ctx = malloc(sizeof(priskv_cluster_req_context));
    priskv_req_ctx->cb = cb;
    priskv_req_ctx->cbarg = cbarg;
    priskv_req_ctx->ignore_no_such_key = 1;

    priskvClusterAsyncDelete(priskv_ctx->client, key, priskv_cluster_req_cb, priskv_req_ctx);
}

static void priskv_cluster_drv_test(void *ctx, const char *key, void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_context *priskv_ctx = ctx;
    priskv_cluster_req_context *priskv_req_ctx = malloc(sizeof(priskv_cluster_req_context));
    priskv_req_ctx->cb = cb;
    ...
    priskvClusterAsyncTest(priskv_ctx->client, key, priskv_cluster_req_cb, priskv_req_ctx);
}

typedef struct {
    priskv_cluster_context *priskv_ctx;
    void (*cb)(int, void *);
    void *cbarg;
    void *transfer_value;
    void *value;
    uint32_t value_len;
} priskv_cluster_req_transfer_context;

static void priskv_cluster_get_transfer_cb(int status, void *arg)
{
    priskv_cluster_req_transfer_context *ctx = arg;
    job_context *job = ctx->priskv_ctx->job;
...
static void priskv_cluster_drv_get_transfer(void *ctx, const char *key, void *value,
                                          uint32_t value_len, void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_req_transfer_context *req_ctx = malloc(sizeof(priskv_cluster_req_transfer_context));
    priskv_cluster_context *priskv_ctx = ctx;
    ...
    priskv_cluster_drv_get(priskv_ctx, key, req_ctx->transfer_value, value_len,
                         priskv_cluster_get_transfer_cb, req_ctx);
}

static void priskv_cluster_set_transfer_cb(int status, void *arg)
{
    priskv_cluster_req_transfer_context *ctx = arg;
...
static void priskv_cluster_drv_set_transfer(void *ctx, const char *key, void *value,
                                          uint32_t value_len, void (*cb)(int, void *), void *cbarg)
{
    priskv_cluster_context *priskv_ctx = ctx;
    job_context *job = priskv_ctx->job;
    ...
    priskv_cluster_drv_set(ctx, key, req_ctx->transfer_value, value_len, priskv_cluster_set_transfer_cb,
                         req_ctx);
}

static const kv_driver priskv_cluster_drv = {
    .name = "priskv-cluster",
    .transfer = false,
    .init = priskv_cluster_drv_init,
    .deinit = priskv_cluster_drv_deinit,
    .get_fd = priskv_cluster_drv_get_fd,
    .handler = priskv_cluster_drv_handler,
    .status_str = priskv_cluster_drv_status_str,
    .get = priskv_cluster_drv_get,
    .set = priskv_cluster_drv_set,
    .del = priskv_cluster_drv_del,
    .test = priskv_cluster_drv_test,
};

static const kv_driver priskv_cluster_drv_transfer = {
    .name = "priskv-cluster",
    .transfer = true,
    .init = priskv_cluster_drv_init,
    .deinit = priskv_cluster_drv_deinit,
    ...
    .handler = priskv_cluster_drv_handler,
    .status_str = priskv_cluster_drv_status_str,
    .get = priskv_cluster_drv_get_transfer,
    .set = priskv_cluster_drv_set_transfer,
    .del = priskv_cluster_drv_del,
    .test = priskv_cluster_drv_test,
};

const kv_driver *kv_drivers[] = {&priskv_drv, &priskv_drv_transfer, &priskv_cluster_drv,
                                 &priskv_cluster_drv_transfer};
